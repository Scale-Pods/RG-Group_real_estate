...
import { getMockMaqsamCalls } from "@/lib/mock-data";
...
export async function GET(request: Request) {
    try {
        ...
        // No credentials configured (demo/mock mode) — return mock CDR rows.
        if (!ACCESS_KEY_ID || !ACCESS_SECRET) {
            return NextResponse.json(getMockMaqsamCalls(parseInt(limit, 10) || 50));
        }
