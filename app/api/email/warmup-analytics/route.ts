...
import { getMockWarmupAnalytics } from '@/lib/mock-data';
...
export async function POST() {
    try {
        ...
        // warmup analytics instead of a 500 so the dashboard still renders.
        if (!apiKey) {
            return NextResponse.json(getMockWarmupAnalytics());
        }
