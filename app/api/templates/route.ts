...
import { getMockTemplates } from '@/lib/mock-data';

export async function GET() {
    try {
        // Tolerate an unreachable n8n instance — a rejected fetch must not 500 the page.
        const okOrNull = (p: Promise<Response>) => p.catch(() => null);
        const [emailRes, whatsappRes] = await Promise.all([
            okOrNull(fetch(process.env.TEMPLATES_EMAIL_WEBHOOK_URL || 'https://n8n.srv1010832.hstgr.cloud/webhook/template-email', { cache: 'no-store' })),
            okOrNull(fetch(process.env.TEMPLATES_WHATSAPP_WEBHOOK_URL || 'https://n8n.srv1010832.hstgr.cloud/webhook/template-whatsapp', { cache: 'no-store' }))
        ]);
        ...
        // Nothing came back from the webhooks (demo/mock mode) — serve the
        // built-in RG Group Dubai template set so the page is never empty.
