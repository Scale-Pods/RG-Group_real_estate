            ...
            >
                ...
                <div style={{ height: 90, padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.3s ease' }}>
                    <div style={{ position: 'relative', width: isExpanded ? 190 : 48, height: isExpanded ? 52 : 48, transition: 'all 0.3s ease', overflow: 'hidden' }}>
                        {isExpanded ? (
                            <Image src="/RG-Group-Logo.png" alt="RG Group Dubai Logo" fill className="object-contain object-center" style={dark ? { filter: 'invert(1) hue-rotate(180deg) brightness(1.5)' } : undefined} priority />
                        ) : (
                            <div className="w-12 h-12 rounded-xl bg-[var(--blue)]/10 text-[var(--blue)] flex items-center justify-center font-bold text-2xl border border-[var(--blue)]/20">R</div>
                        )}
                    </div>
                </div>
                ...
                <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: 8, borderTop: '1px solid var(--hairline)', background: 'var(--bg-layer1)' }}>
                    ...
                    <button
                        onClick={async () => { await logout(); window.location.href = 'https://rg-group-automations.vercel.app/'; }}
                        style={{
                            display: 'flex', alignItems: 'center', gap: 12,
                            padding: '10px 12px', borderRadius: '12px',
                    ...
                    {isExpanded && (
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: '10px', color: 'var(--label-quaternary)', marginTop: 8, letterSpacing: '0.05em' }}>
                            ...
                            <Image src="/scalepods-logo.avif" alt="ScalePods" width={60} height={16} style={{ filter: 'brightness(0.9) opacity(0.8)', width: 'auto', height: 16 }} />
                        </div>
                    )}
                </div>
            </aside>
