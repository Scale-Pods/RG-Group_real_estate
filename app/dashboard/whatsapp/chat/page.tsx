...
export default function WhatsappChatPage() {
    ...
    const introLeads = useMemo(() => filteredLeads.filter(l => l.source_loop === "Intro"), [filteredLeads]);
    const nurtureLeads = useMemo(() => filteredLeads.filter(l => l.source_loop === "Nurture"), [filteredLeads]);
    const activeLeads = activeTab === "intro" ? introLeads : nurtureLeads;
                    ...
                    <div style={{ display: 'flex', background: 'var(--fill-tertiary)', borderRadius: 'var(--radius-md)', padding: 3, gap: 2 }}>
                        <button
                            ...
                            style={{
                                ...
                                background: activeTab === "intro" ? 'var(--bg-layer1)' : 'transparent',
                                color: activeTab === "intro" ? 'var(--label-primary)' : 'var(--label-secondary)',
                                boxShadow: activeTab === "intro" ? 'var(--shadow-sm)' : 'none',
                            }}
                        >
                            <Users style={{ width: 13, height: 13 }} />Intro Loop
                        </button>
                        <button
                            onClick={() => { setActiveTab("nurture"); setCurrentPage(1); }}
                            style={{
                                padding: '5px 14px', borderRadius: 'var(--radius-sm)',
                                fontSize: 12, fontWeight: 600, border: 'none', cursor: 'default',
                                display: 'flex', alignItems: 'center', gap: 5, transition: 'all 130ms',
                                background: activeTab === "nurture" ? 'var(--bg-layer1)' : 'transparent',
                                color: activeTab === "nurture" ? 'var(--green)' : 'var(--label-secondary)',
                                boxShadow: activeTab === "nurture" ? 'var(--shadow-sm)' : 'none',
                            }}
                        >
                            <MessageSquare style={{ width: 13, height: 13 }} />Nurture Loop
                        </button>
                    </div>
